...
/**
 * Đồng bộ thiepcuoi4 từ info.json và images-update4.
 *
 * Chạy tại thư mục chứa file này:
 *   node update-thiepcuoi4.mjs
 *   node update-thiepcuoi4.mjs --dry-run
 *   node update-thiepcuoi4.mjs --info /duong-dan/info-khac.json
 *
...
Tool cập nhật thiepcuoi4/index.html, đồng bộ 15 ảnh photo-01.jpg đến
photo-15.jpg và hai QR qr_chure.jpg, qr_codau.jpg từ images-update4.`);
  process.exit(0);
}
...
const invitationDir = resolve(toolDir, 'thiepcuoi4');
...
async function validateVendor() {
  ...
  if (missing.length) {
    throw new Error(`Thiếu ${missing.length} tài nguyên cục bộ trong thiepcuoi4/assets/vendor: ${missing.join(', ')}.`);
  }
}
...
async function main() {
  ...
  console.log(`${dryRun ? '[dry-run] ' : ''}Đã ${dryRun ? 'kiểm tra' : 'cập nhật'} thiepcuoi4.`);
  ...
  console.log(`- Ảnh cá nhân: ${photoCount} ảnh JPEG + 2 QR trong thiepcuoi4/assets/images.`);
