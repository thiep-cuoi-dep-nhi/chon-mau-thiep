...
/**
 * Đồng bộ thiepcuoi5 từ info.json và images-update5.
 *
 * Chạy tại thư mục chứa file này:
 *   node update-thiepcuoi5.mjs
 *   node update-thiepcuoi5.mjs --dry-run
 *   node update-thiepcuoi5.mjs --info /duong-dan/info-khac.json
 *
 * Mẫu 5 dùng 15 ảnh photo và 2 mã QR từ bộ ảnh riêng của mẫu 5.
 */
...
Tool cập nhật thiepcuoi5/index.html, đồng bộ 15 ảnh photo-01.jpg đến
photo-15.jpg và hai QR qr_chure.jpg, qr_codau.jpg từ images-update5.`);
  process.exit(0);
}
...
const invitationDir = resolve(toolDir, 'thiepcuoi5');
const htmlPath = resolve(invitationDir, 'index.html');
const sourceImagesDir = resolve(toolDir, 'images-update5');
...
function insertGiftEntry(html) {
  html = html.replace(/<!-- thiepcuoi(?:3|5)-gift-entry:start -->[\s\S]*?<!-- thiepcuoi(?:3|5)-gift-entry:end -->/g, '');
  const entry = `<!-- thiepcuoi5-gift-entry:start -->
<div id="SECTION_GIFT_ENTRY" class="ladi-section">
  <div class="ladi-container">
    <button id="GIFT_ENTRY_BUTTON" type="button">Gửi mừng cưới</button>
  </div>
</div>
<!-- thiepcuoi5-gift-entry:end -->`;
...
function insertLocalMusic(html) {
  ...
  html = html.replace(/<!-- thiepcuoi(?:3|5)-music:start -->[\s\S]*?<!-- thiepcuoi(?:3|5)-music:end -->/g, '');
  const music = `<!-- thiepcuoi5-music:start -->
<style id="thiepcuoi5-music-style">
  #thiepcuoi5-music-toggle {
    position: fixed; z-index: 1000000001; left: 20px; bottom: 20px;
    width: 50px; height: 50px; border: 0; border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    background: #9f621d; color: #fff; box-shadow: 0 5px 14px rgba(0, 0, 0, .28);
    cursor: pointer; -webkit-tap-highlight-color: transparent;
  }
  #thiepcuoi5-music-toggle .music-icon { font: 28px/1 Arial, sans-serif; transform: translateY(-1px); }
  #thiepcuoi5-music-toggle.is-playing { animation: thiepcuoi5-music-pulse 1.8s ease-in-out infinite; }
  @keyframes thiepcuoi5-music-pulse { 50% { box-shadow: 0 0 0 8px rgba(159, 98, 29, .16), 0 5px 14px rgba(0, 0, 0, .28); } }
</style>
<audio id="thiepcuoi5-background-music" loop preload="metadata">
  <source src="assets/vendor/i-do-911.mp3" type="audio/mpeg">
</audio>
<button id="thiepcuoi5-music-toggle" type="button" title="Bật nhạc" aria-label="Bật nhạc" aria-pressed="false">
  <span class="music-icon" aria-hidden="true">♪</span>
</button>
<script id="thiepcuoi5-music-runtime">
(() => {
  const audio = document.getElementById('thiepcuoi5-background-music');
  const button = document.getElementById('thiepcuoi5-music-toggle');
...
function localizePersonalImages(html) {
  ...
  if (new Set(directImages.values()).size !== directImages.size) {
    throw new Error('Mapping ảnh trực tiếp của mẫu 5 bị trùng; mỗi vị trí phải dùng một ảnh riêng.');
  }
...
function removePreviousRuntime(html) {
  return html.replace(/<!-- thiepcuoi(?:3|5)-runtime:start -->[\s\S]*?<!-- thiepcuoi(?:3|5)-runtime:end -->/g, '');
}

function clientRuntime(data) {
  const payload = JSON.stringify(data).replaceAll('<', '\\u003c').replaceAll('</script', '<\\/script');
  return `<!-- thiepcuoi5-runtime:start -->
<style id="thiepcuoi5-runtime-style">
  /* Mẫu 5 dùng phong bì vàng champagne và con dấu bạc đã xử lý trong PNG. */
  ...
  #HEADLINE21 > .ladi-headline { font-family: QWxleEJydXNoLVJlZVsYXIudHRm !important; font-size: 38px !important; font-weight: 400 !important; font-style: normal !important; line-height: 1.25 !important; color: rgb(159, 98, 29) !important; text-decoration: none !important; text-align: center !important; }
  ...
  #BUTTON7 > .ladi-button > .ladi-button-background, #BUTTON9 > .ladi-button > .ladi-button-background { background-color: #9f621d !important; }
  ...
  #GIFT_ENTRY_BUTTON { border: 0; border-radius: 26px; min-width: 224px; padding: 14px 28px; background: #9f621d; color: #fff; font: 700 18px "Maven Pro", sans-serif; cursor: pointer; box-shadow: 0 8px 18px rgba(159, 98, 29, .28); }
  #GIFT_ENTRY_BUTTON:active { transform: translateY(2px); }
  #GIFT_POPUP_CLOSE { position: absolute; z-index: 9000000090; top: 8px; right: 8px; width: 30px; height: 30px; border: 0; border-radius: 50%; background: #9f621d; color: #fff; font-size: 24px; line-height: 26px; cursor: pointer; }
</style>
<script id="thiepcuoi5-runtime">
(() => {
  const data = ${payload};
...
async function validateVendor() {
  ...
  if (missing.length) {
    throw new Error(`Thiếu ${missing.length} tài nguyên cục bộ trong thiepcuoi5/assets/vendor: ${missing.join(', ')}.`);
  }
}
...
async function syncImages() {
  ...
  if (missing.length) {
    throw new Error(`images-update5 thiếu: ${missing.join(', ')}. Mẫu 5 cần 15 ảnh photo và 2 QR.`);
  }
...
async function main() {
  ...
  console.log(`${dryRun ? '[dry-run] ' : ''}Đã ${dryRun ? 'kiểm tra' : 'cập nhật'} thiepcuoi5.`);
  ...
  console.log(`- Ảnh cá nhân: ${photoCount} ảnh JPEG + 2 QR trong thiepcuoi5/assets/images.`);
  ...
  if (unavailableFields.length) {
    console.log(`- Lưu ý: mẫu 5 không có vùng hiển thị riêng cho ${unavailableFields.join(', ')}; các trường này không được đưa lên thiệp.`);
  }
}
